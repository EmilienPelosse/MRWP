...
def showGraphDynamic(G, data, positions, filename):
    ...
    node_trace = []
    # computation of the list of colour of each node col[n] will be the colour
    # of the nth node in the current frame
    for t in range(tMax):
        col = []
        for n in G:
    ...
    frames = [{'data': [edge_trace, node_trace[k]],
               'layout':{
        # change this if you want to change the information visualized at the
        # top of the page
...
def showData(data, filename):
    ...
    # slice the coloumns of the given matrix (note that the input is just a
    # matrix 3Xn)
    y = data[:, 0]
    y1 = data[:, 1]
    y2 = data[:, 2]
